...
def keyboard_events(nose_cords, cords, img):
    try:
        [(x1,y1), (x2,y2)] = cords
        xc, yc = nose_cords
    except: 
    ...
    if cmd:
        ...
        cv2.putText(img, cmd, (10,50), cv2.FONT_HERSHEY_SIMPLEX, 3, color['red'], 1, cv2.LINE_AA)
    return img
def reset_press_flag(nose_cords, cords):
    try:
        [(x1,y1), (x2,y2)] = cords
        xc, yc = nose_cords
    except: 
...
while True:
    ...
    if press_flag:
        img = keyboard_events(nose_cords,cords, img)
        press_flag = False
    press_flag = reset_press_flag(nose_cords,cords)
